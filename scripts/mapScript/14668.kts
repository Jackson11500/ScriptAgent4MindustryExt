...
class TechNode(
    ...
    suspend fun build(player: Player, core: CoreBuild, team: Team = player.team()): Pair<String, suspend () -> Unit>{
    ...
    suspend fun buildDesc(player: Player, core: CoreBuild, team: Team = player.team()): Pair<String, suspend () -> Unit>{
...
val techTree by autoInit{ TechNode(tech = fun Player.():Pair<() -> Unit, String> { return ({} to "[green]科技树")}, alwaysActive = true) }
...
fun Player.lastTeam(): Team{ return playerLastTeam.getOrDefault(uuid(), team()) }
fun Player.lastTeam(lastTeam: Team){  playerLastTeam[uuid()] = lastTeam }
...
fun CoreBuild.lord(): List<String> { return cityData().lord }
fun CoreBuild.lord(uuid: String) { if (!lord().contains(uuid)) cityData().lord.add(uuid)}
...
class MegaStructure(
    ...
    fun status(team: Team): String {
        return "${if (this in team.techData().megaStructureQueue) "[yellow]" else (if (this in team.techData().megaStructures) "[cyan]" else if (this.canBuild(team)) "[green]" else "[red]")}■"
    }
}

class MegaStructures(
    val airDrop: MegaStructure = MegaStructure("[cyan]轨道空降", "[green]发射台可以花10s传送至任意地点", mutableMapOf(
        Items.copper to 80000,
        Items.lead to 40000
    )),
    val superComputer: MegaStructure = MegaStructure("[cyan]星空算机", "[green]每秒提供大量科技经验\n建成后获得2点科技点", mutableMapOf(
    ...
    val satellite: MegaStructure = MegaStructure("[cyan]间谍卫星", "[green]永久开启虚空全知超武\n敌方影之猎杀科技对你无效\n你的影之猎杀科技不需要迷雾即可触发", mutableMapOf(
    ...
    val withdrawal: MegaStructure = MegaStructure("[purple]虚空抽取", "[green]为队伍银行提供相当于5个首都城市的收入", mutableMapOf(
        Items.copper to 80000,
        Items.lead to 80000
    ), fun Team.() {
        loop(Dispatchers.game) {
    ...
    val contract: MegaStructure = MegaStructure("[purple]虚空契约", "[green]获得10点科技点\n[red]但清算终将到来", mutableMapOf(
        Items.copper to 80000,
        Items.lead to 80000
    ), fun Team.() {
        val team = this
    ...
    val godStick: MegaStructure = MegaStructure("[yellow]上帝权杖", "[green]每5秒对一个敌对玩家造成5000点伤害,受50倍护甲削减", mutableMapOf(
        Items.copper to 120000,
        Items.lead to 80000
    ), fun Team.() {
        val team = this
        loop(Dispatchers.game) {
            Groups.player.filter { it.team() != team && !it.dead() }.random().apply {
    ...
    val nanoMachine: MegaStructure = MegaStructure("[yellow]纳米重铸", "[green]永久开启回生超武", mutableMapOf(
        Items.copper to 60000,
        Items.lead to 60000
    ), fun Team.() {
...
fun CoreBuild.levelText(noColor: Boolean = false): String {
    return "[white]${if (!noColor && coins() >= maxHealth * team().techData().cityUpgradeMultiplier && block != Blocks.coreAcropolis) "${Iconc.up}[cyan]" else "${if (!noColor) Iconc.upload else ""}"}${when (level()) {
        1 -> "村庄"
        2 -> "乡镇"
        3 -> "庄园"
        4 -> "城市"
    ...
    unit.apply {
        ...
        launch(Dispatchers.game){
            ...
            while(Time.timeSinceMillis(spawnTime) / 1000f <= 120f * team.techData().lordUnitSpawnTimeMultiplier){
                Call.label("${unit.type.emoji()}[#${team().color}]领主级单位降临[white]${unit.type.emoji()}${if(core.tile.build !is CoreBuild || core.tile.build.team != this@createLordUnit.team()) "  [red]被打断！[white]" else ""}\n$name [white]${(120f * team.techData().lordUnitSpawnTimeMultiplier - Time.timeSinceMillis(spawnTime) / 1000f).format()}", 0.2026f, x, y)
            ...
            Call.logicExplosion(team, x, y, 40f * 8f * team.techData().lordUnitExplosionMultiplier, 7500f * team.techData().lordUnitExplosionMultiplier, true, true, true)
                ...
                if (core.dead || !core.isValid || core.team() != team()){
                    ...
                    if (checkCooldown()) {
                        ...
                        if (!unit().spawnedByCore)
                            amount = (core.coins() * Random.nextDouble(if(team().techData().alphaMoreCoins) 0.50 else 0.00, 1.00)).toInt()
                        else
                            amount = (core.coins() * Random.nextDouble(if(team().techData().alphaMoreCoins) 0.25 else 0.00, if(team().techData().alphaMoreCoins) 0.50 else 0.25)).toInt()
                        addCoin(amount)
                            ...
                            if (coins() >= (core.maxHealth * team().techData().cityUpgradeMultiplier / 4f) && core == (core.tile.build as CoreBuild)) {
                                ...
                                Groups.player.forEach {
                                    if (it.team() == team() || fogControl.isVisible(
                                        ...
                                        it.sendMessage(
                                            "$name [#${core.team.color}] 已经加入位于[${World.toTile(core.x)},${World.toTile(core.y)}]的 [white]${core.block.emoji()} [#${core.team.color}]的领主"
                                        )
                                }
                            }
                        })
...
suspend fun Player.cityTypeMenu(core: CoreBuild) {
    ...
    menu.sendMenuBuilder<Unit>(
        ...
        CityTypes.toArray().forEach { typesArray ->
            ...
            typesArray.forEach { if (core.cityData().cityType.name == "[white]均衡城市" && (it.name != "[cyan]军工补给经济城市" || team().techData().WLECheaper)) list.add(cityType(it, 0.45f)) else list.add(cityType(it))}
            add(list)
        }
                    ...
                    if (!core.isValid || core.team() != team()){
                        ...
                        if (coins() >= buyUnitCost() - (if (team().techData().allCoinsAreUnits) buyUnitCost() / 2 else 0) && core.cityData().population > unitType().level() - 1) {
                            if (createUnit(core)) {
                                ...
                                core.cityData().population -= Random.nextInt(unitType().level() * 4).coerceAtMost(core.cityData().population)
                                warMenu(core)
                            } else {
...
suspend fun Player.techTreeMenu(core: CoreBuild, node: TechNode) {
    ...
    menu.sendMenuBuilder<Unit>(
        this, 900_000, "[green]科技页面\n[cyan]City-level[yellow]${core.level()}", "[cyan]科技树[white]\n" + whileBuildName(node)
    ) {
        playerInputing[uuid()] = false
        if (node.parent() != null)
            add(listOf(node.parent()!!.buildDesc( this@techTreeMenu, core)))
        add(buildList {
            add("${if (node.parent() != null) "[cyan]------↑父科技------\n" else ""}[yellow]------↓此科技------" to {techTreeMenu(core, node)})
        })
        add(listOf(node.buildDesc( this@techTreeMenu, core)))
        add(buildList {
            add("[yellow]------↑此科技------${if (!node.child().isNullOrEmpty()) "\n[green]------↓子科技------" else ""}" to {techTreeMenu(core, node)})
        })
...
onEnable{
    ...
    contextScript<coreMindustry.UtilMapRule>().apply {
        ...
        if (UnitTypes.disrupt.weapons.get(0).bullet.spawnUnit.weapons.get(0).bullet.spawnUnit != UnitTypes.quell.weapons.get(0).bullet.spawnUnit)
            registerMapRule(
  ...
  "unit": {
    ...
    loop(Dispatchers.game){
        state.rules.modeName = "${randomColor()}Lord${randomColor()}Of${randomColor()}War"
        delay(100L)
    }
    loop(Dispatchers.game){
        Call.setRules(state.rules)
    ...
    launch(Dispatchers.game){
        ...
        val commonTech = buildList {
            repeat(5){add(fun Player.(): Pair<() -> Unit, String> {
                return fun () {
            ...
            repeat(7){add(fun Player.(): Pair<() -> Unit, String> {
                return fun () {
        ...
        val voidTech = buildList {
            ...
            add(fun Player.(): Pair<() -> Unit, String> {
                return fun () {
                    ...
                    val lastCore: CoreBuild = Groups.build.toList().filter { it.team == team() && it.block is CoreBlock }.random() as CoreBuild
                    Groups.build.toList().filter { it.team == team() && it.block is CoreBlock && it != lastCore }.forEach {
                                            ...
                                            node8.addChild(buildList {
                                                ...
                                                if (tech9 == epicTech[1])
                                                    node9.addChild(buildList {
                                                        ...
                                                        val vNode10 = TechNode(tech = vTech10)
                                                        add(vNode10)
    ...
    loop(Dispatchers.game){
        ...
        state.teams.getActive().forEach { it ->
            ...
            it.team.addCoin(if (it.team.techData().hedgeFund) abs(rate.toInt()) * 5 else rate.toInt().coerceAtLeast(if (it.team.techData().bankNoRemoveCoins) 0 else Int.MIN_VALUE))
            ...
            it.cores.forEach { c ->
                if (c.cityData().cityType.name == "[white]均衡城市" && Random.nextFloat() <= 0.001 && it.team.techData().autoSetCityType) {
                    val type = arrayOf(CityTypes.War, CityTypes.Economic, CityTypes.Logistics, CityTypes.ResearchI).random()
                ...
                val expAmount = (c.cityData().cityType.researchMultiplier - 1f) * it.team.techData().level * c.level() / 3f + c.level()
                it.team.techData().exp += expAmount
                c.addCoin(amount)
                c.cityData().population += (c.level() * Random.nextFloat() * (1f + c.coins() / (c.maxHealth * 0.4f)) * (1f + (1f - c.cityData().cityType.unitCostMultiplier) * 5f).coerceAtLeast(1f)).toInt().coerceAtLeast(1).coerceAtMost((((c.level() + 1f).pow(3)) * (1f + (1f - c.cityData().cityType.unitCostMultiplier) * 5f).coerceAtLeast(1f)).toInt() - c.cityData().population)
                val allyText = buildString {
                    appendLine("[#${c.team.color}]${Iconc.blockCliff}${c.coins()} ${Iconc.players}${c.cityData().population}/${((c.level() + 1f).pow(3) * (1f + (1f - c.cityData().cityType.unitCostMultiplier) * 5f).coerceAtLeast(1f)).toInt()}")
                ...
                Groups.player.filter { p ->
                    ...
                    Call.label(p.con, if (p.team() == it.team || p.team().techData().knowOtherTeamInfo >= Time.millis()) allyText else enemyText, 1.013f, c.x, c.y)
                }
    ...
    loop(Dispatchers.game) {
        Team.all.forEach { team ->
            if (team.techData().teamRespawn && !team.techData().teamRespawnUsed && team.data().noCores() && !state.gameOver) {
                ...
                Call.logicExplosion(team, tile.worldx(), tile.worldy(), 40f * 8f, 1500f, true, true, true)
        ...
        state.teams.getActive().forEach { t ->
            ...
            accelerators.forEach { (it as AcceleratorBuild)
                if (it.full()) {
                ...
                Call.label(
                    ...
                    [white]${Iconc.itemCopper} ${it.items.get(Items.copper)}/${(it.block as Accelerator).launching.requirements[0].amount}
                    [white]${Iconc.itemLead} ${it.items.get(Items.lead)}/${(it.block as Accelerator).launching.requirements[1].amount}
            ...
            pads.forEach padsForEach@{
                ...
                Groups.player.filter { p ->
                    ...
                    Call.label(p.con, if (p.team() == it.team || p.team().techData().knowOtherTeamInfo >= Time.millis()) allyText else enemyText, 1.013f, pad.x, pad.y)
                }
            }
        }
        delay(1000)
    ...
    loop(Dispatchers.game){
        Groups.player.forEach{
            val text = buildString {
                ...
                appendLine("单位上限:${Groups.unit.toList().filter { u -> u.owner() == it.uuid() }.size}/${it.unitCap()}")
                if (!it.checkCooldown())
            ...
            if (megaStructure.status(team()) == "[green]■" && launcher.isValid && !launcher.launching()) {
                ...
                launch(Dispatchers.game) {
                    ...
                    while (Time.timeSinceMillis(launchTime) < megaStructure.cost()) {
                        ...
                        Call.label("${megaStructure.name}[#${team.color}]正在发射！\n${(Time.timeSinceMillis(launchTime) / 1000f).format()}s/${(megaStructure.cost() / 1000f).format()}s", 0.7026f, launcher.x, launcher.y)
...
suspend fun Player.launchPadMenu(pad: LaunchPad.LaunchPadBuild) {
    ...
    menu.sendMenuBuilder<Unit>(
        this, 900_000, "[green]发射台页面 [#${pad.team.color}]${pad.padData().name}\n[cyan]发射台总数[yellow]${pads.size}",
        """
            ...
            if (pads.size >= 2) {
                if (targetPad != null) {
                    ...
                    menu.sendMenuBuilder<Unit>(
                        ...
                        pads.filter{ it != pad }.sortedBy { if ((it as LaunchPad.LaunchPadBuild).padData().showInfo) 0 else 1 }.forEach {
                ...
                this += listOf("[cyan]传送你附近的军团单位至指定目标点！\n[red]会有强大的副作用" to {
                    launch(Dispatchers.game) {
                        ...
                        launch(Dispatchers.game) b@{
                            val units = buildList { Units.nearby(team(), pad.x, pad.y, itemTransferRange) {
                                add(it)
                                if (it.owner() == uuid() || it == unit())
...
fun AcceleratorBuild.full(): Boolean {
    return (block as Accelerator).launching.requirements.all { items.get(it.item) >= it.amount }
}
fun AcceleratorBuild.full(item: Item): Boolean {
    return items.get(item) >= (block as Accelerator).launching.requirements[item.id.toInt()].amount
}
suspend fun Player.superWeaponsMenu(accelerator: AcceleratorBuild) {
    menu.sendMenuBuilder<Unit>(
        ...
        """
            [red]指向性超级武器需要视野！
            [white]${Iconc.itemCopper} ${accelerator.items.get(Items.copper)}/${(accelerator.block as Accelerator).launching.requirements[0].amount}
            [white]${Iconc.itemLead} ${accelerator.items.get(Items.lead)}/${(accelerator.block as Accelerator).launching.requirements[1].amount}
        """.trimIndent()
    ) {
        playerTapping[uuid()] = false
        if (accelerator.full()) {
                    ...
                    add(
                        "[navy]虚空全知\n[white]可查看其他队伍的城市与传送台情报！\n持续1200s" to {
                            if (accelerator.isValid && accelerator.team == team()) {
                                ...
                                """.trimIndent())
                                team().techData().knowOtherTeamInfo = if (team().techData().knowOtherTeamInfo <= Time.millis()) Time.millis() + 1200_000 else team().techData().knowOtherTeamInfo + 1200_000
                                accelerator.kill()
                            ...
                            if (accelerator.isValid && accelerator.team == team()) {
                                ...
                                launch(Dispatchers.game) {
                                    ...
                                    fun mindustry.gen.Unit.inEnemyArea(): Boolean{
                                        ...
                                        return team != team() && closestCore.team == team && (state.rules.polygonCoreProtection || dst(closestCore) < state.rules.enemyCoreBuildRadius)
                                    ...
                                    while (Time.timeSinceMillis(lockDownStartTime) <= 240_000){
                                        Groups.unit.forEach {
                                            if (it.inEnemyArea()) {
                                                if (it.isPlayer)
                                                    Call.infoToast(it.player.con, "[red]你已经进入虚空领域！\n在你离开之前每秒受到最大生命12%伤害", 1.013f)
                                        ...
                                        Groups.unit.forEach {
                                            if (it.inEnemyArea()) {
                                                ...
                                                if (!it.hasEffect(StatusEffects.slow) && it.isPlayer)
                                                    Call.announce(it.player.con, "[red]你已经进入封锁区！\n在你离开之前获得大量debuff")
                        ...
                        if (accelerator.isValid && accelerator.team == team()) {
                            ...
                            if (!fail) {
                                ...
                                while (Time.timeSinceMillis(startTickingTime) <= 10_000) {
                                    Call.label("[#${team.color}]检测到在途的轰击打击!\n[white]${(10f - Time.timeSinceMillis(startTickingTime) / 1000f).format()}", 0.513f, tile.worldx(), tile.worldy())
                                ...
                                Call.logicExplosion(team, tile.worldx(), tile.worldy(), 96f * 8f, 50f, true, true, true)
                                Call.logicExplosion(Team.derelict, tile.worldx(), tile.worldy(), 48f * 8f, 500f, true, true, false)
                                Call.logicExplosion(Team.derelict, tile.worldx(), tile.worldy(), 32f * 8f, 2000f, true, true, false)
                                Call.logicExplosion(Team.derelict, tile.worldx(), tile.worldy(), 12f * 8f, 5000f, true, true, false)
                                Call.logicExplosion(Team.derelict, tile.worldx(), tile.worldy(), 6f * 8f, 32000f, true, true, false)
                                Call.soundAt(Sounds.explosionbig, tile.worldx(), tile.worldy(), 114514f, 0f)
                                Call.effect(Fx.impactReactorExplosion, tile.worldx(), tile.worldy(), 0f, team.color)
                                var delayTime: Long
                                delayTime = Random.nextLong(20, 100)
                                Call.label("[#${team.color}]检测到在途的轰击打击!\n[white]200/200", delayTime / 1000f * 1.2f, tile.worldx(), tile.worldy())
                                repeat(200){
                                    delay(delayTime)
                                    launch(Dispatchers.game) {
                                        Tmp.v1.rnd(Random.nextFloat() * 96f * 8f)
                                        ...
                                        Call.soundAt(Sounds.place, sx, sy, 3f, 0f)
                                        delay(Random.nextLong(1_000, 2_000))
                                        Call.logicExplosion(Team.derelict, sx, sy, 32f * 8f, 200f, true, true, false)
                                        Call.logicExplosion(Team.derelict, sx, sy, 12f * 8f, 500f, true, true, false)
                                        Call.logicExplosion(Team.derelict, sx, sy, 6f * 8f, 1500f, true, true, false)
                                        Call.soundAt(Sounds.explosionbig, tile.worldx(), tile.worldy(), 1.5f, 0f)
                                        Call.effect(Fx.greenBomb, sx, sy, 0f, team.color)
                                    }
                                    delayTime = Random.nextLong(20, 100)
                                    Call.label("[#${team.color}]检测到在途的轰击打击!\n[white]${200 - it}/200", delayTime / 1000f * 1.2f, tile.worldx(), tile.worldy())
                                }
                                if(bak) {
        ...
        if (admin && Groups.player.size() <= 8){
            this += listOf(
                "[red]<ADMIN>FULL" to { accelerator.items.add((accelerator.block as Accelerator).launching.requirements.toMutableList()) }
            )
        }
        this += listOf(
            "取消" to {}
...
listen<EventType.UnitDamageEvent> { e ->
    ...
    val float = ((bullet.type.damage / ((bullet.owner as mindustry.gen.Unit).type?.level() ?: 1) * 10f)).coerceAtLeast(0.1f).coerceAtMost(1f)
    ...
    if ((unit.team().techData().voidAnnihilation && Random.nextFloat() <= 0.01f) || (bullet.team().techData().voidAnnihilation && Random.nextFloat() <= 0.02f)){
        unit.maxHealth /= 2f
        unit.clampHealth()
    ...
    if(
        ((bullet.team.techData().fogKiller && !fogControl.isVisible(unit.team, (bullet.owner as mindustry.gen.Unit).x, (bullet.owner as mindustry.gen.Unit).y)) || MegaStructures.satellite in bullet.team.techData().megaStructures) && MegaStructures.satellite !in bullet.team.techData().megaStructures
...
listen<EventType.UnitDestroyEvent> {
    ...
    if (it.unit.team.techData().deadBoom){
        launch(Dispatchers.game) {
            ...
            Call.logicExplosion(unit.team, unit.x, unit.y, 8f * 8f, unit.maxHealth / 10, true, true, true)
        }
    }
}
...
listenPacket2Server<UnitControlCallPacket> {con, packet ->
    ...
    if (owner == null){
        ...
        if (con.player.uuid() != owner) {
            if (!unit.team.techData().controlOtherPlayerUnits && unit.type !in LordUnits) Call.announce(con, "[red]你不是该单位的领主！无法控制")
