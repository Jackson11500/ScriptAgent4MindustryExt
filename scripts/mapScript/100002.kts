    ...
    ts.launch(Dispatchers.game) {
        while (true) {
            if (u.tileOn() != null && u.tileOn().passable() && u.tileOn()?.floor()?.isDeep != true) {
                ...
                u.apply(StatusEffects.fast, 5 * 60f)
...
val lv1Rules = listOf(
        ...
        Rule("[green][直破苍穹]", "[red]敌方苍穹出厂获得两层迅捷60s") {
            ...
            ts.loop(Dispatchers.game) {
                Team.crux.data().units.filter { it.type == UnitTypes.zenith && !applyMap.getOrDefault(it, false) }.forEach {
                    repeat(2) { i ->
                        it.statuses.add(StatusEntry().set(StatusEffects.fast, 60 * 60f))
                        applyMap[it] = true
                    }
                }
                yield()
...
val lv2Rules = listOf(
        ...
        Rule("[purple][升阳之时]", "[red]敌方苍穹出厂获得保护30s，苍穹袭击数量+1") {
            zenithStrike += 1
            val applyMap = mutableMapOf<mindustry.gen.Unit, Boolean>()
            ts.loop(Dispatchers.game) {
                Team.crux.data().units.filter { it.type == UnitTypes.zenith && !applyMap.getOrDefault(it, false) }.forEach {
                    repeat(2) { i ->
                        it.statuses.add(StatusEntry().set(StatusEffects.shielded, 30 * 60f))
                        applyMap[it] = true
                    }
                }
                yield()
    ...
    launch(Dispatchers.game) {
        ...
        loop(Dispatchers.game) {
            Call.setHudText(buildString {
                ...
                if (exInfo) {
                    appendLine()
                    append("[yellow]距离律令能量耗尽还有:  ${((defStartTime + 30_000L - Time.millis()) / 1000f).format(2)}s")
                }
            })
            delay(500)
        }
        ...
        loop(Dispatchers.game) {
            ...
            spawnDelay = (spawnDelay - 2_000L).coerceAtLeast(30_000L)
            delay(spawnDelay)
        }
        delay(2000)
        ...
        delay(3_000)
        Call.sendMessage(nameR + "场面失控了呢，哥哥")
        delay(4_000)
        ...
        val leftLv3Rules = lv3Rules.toMutableList()
            ...
            while (true) {
                if (ruleExp >= 100f) {
                    ...
                    when (ruleLevel) {
                        ...
                        in 7..9 -> {
                            randomRule = leftLv3Rules.random()
                            leftLv3Rules.remove(randomRule)
                            applyRule(randomRule)
                        }
                    }
                    ruleLevel += 1
        ...
        applyRule(Rule("[双生召唤]", "[lightgray]我们同生，共死。") {
            ts.launch(Dispatchers.game) {
                ...
                Vars.world.tiles.forEach {
                    if (it.floor() == Blocks.cryofluid) {
                        launch(Dispatchers.game) {
                            delay((100_000 * Random.nextFloat()).toLong())
        ...
        applyRule(randomRule)
        delay(4_000L)
        Call.sendMessage(name + "始源铭和终焉铭..是啊..总是相伴在一起..")
        repeat(8) {
        ...
        delay(4_000L)
        Call.sendMessage(nameR + "我也要和哥哥一块❤")//你妈的，我自己都要绷不住了，为什么我要写妹控设定.jpg
        ...
        worldLabelMessage(nameRU.x, nameRU.y, "哥哥还是那么相信他的落日理论呢...", 4, "[yellow]")
        delay(4_000L)
        ...
        var triggerNum = 0
        while (true) {
            if (defStartTime + 5_000L <= Time.millis() && triggerNum <= 0) {
            ...
            if (defStartTime + 10_000L <= Time.millis() && triggerNum <= 1) {
                Call.sendMessage(name + "侦测到更多陆军准备登陆!")
            ...
            if (defStartTime + 15_000L <= Time.millis() && triggerNum <= 2) {
            ...
            if (defStartTime + 20_000L <= Time.millis() && triggerNum <= 3) {
                applyRule(Rule("[最终决战]", "[red]无法进行核心重刻") {
                    coreRebuild = false
                })
                triggerNum += 1
            }
            if (defStartTime + 25_000L <= Time.millis() && triggerNum <= 4) {
                applyRule(Rule("[孤注一掷]", "[red]陆军登录无冷却时间") {
                    waveTime = 0L
                })
                triggerNum += 1
            }
            if (defStartTime + 30_000L <= Time.millis()) {
                break
            }
        }
        //FINAL
