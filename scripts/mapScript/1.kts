...
class RuleMenu(private val player: Player): MenuBuilder<Unit>() {
    ...
    private suspend fun mainMenu() {
        ...
        buildWorld("领主战争之域", "[cyan]LordOfWar模式!", 14668, listOf(13545, 100061, 100062, 200003))
        newRow()
        buildWorld("俘获之域", "[cyan]单位被摧毁后会被转化为其他队伍单位!", 100051)
        newRow()
        ...
        newRow()
        buildWorld("美人一小块地", "[cyan]CoreWar模式!", 13545, listOf(14668, 200003))
        newRow()
        ...
        newRow()
        buildWorld("简单世界", "[cyan]启用简单模式!\n[yellow]工厂自动填装", 100111)
        newRow()
        buildWorld("加速世界", "[cyan]WRYYYYY", 100121)
        newRow()
        buildWorld("未曾启蒙之域", "[cyan]启用liteCampaign模式", 200003, listOf(14668, 13545))
        newRow()
        buildWorld("水漫金山之域", "[cyan]启用floodv2模式", 200002)
    }
